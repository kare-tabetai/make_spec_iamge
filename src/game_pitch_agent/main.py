...
def save_pitch_files(
    ...
    # Markdown保存（画像が生成されなかった場合のみ。画像ありの場合はrender_pitch_imageで保存済み）
...
def _log_summary(saved_files: list[dict], output_dir: Path, skip_image: bool = False) -> None:
    """生成結果のサマリーをログ出力する"""
    logger.info("\n" + "=" * 60)
    logger.info("完了！")
...
async def async_render(args: argparse.Namespace) -> int:
    ...
    logger.info("\n" + "=" * 60)
    logger.info(f"レンダリング完了: {rendered_count}/{len(pitch_dirs)} 件")
    logger.info("=" * 60)
...
async def async_full(args: argparse.Namespace) -> int:
    ...
    _log_summary(saved_files, output_dir, skip_image=skip_image)
...
使用例:
  ...
  uv run game-pitch full --topic "お題:「不自由」" --no-image
    ...
    render_parser.add_argument("--force", action="store_true", help="既存画像を上書き再生成")
